...
// Genshin Assets imported from assets.js
// Make sure assets.js is loaded before this script
...
// 1. Handle CSV file reading - Allow manual upload
...
window.addEventListener("load", function () {
  ...
  const basePath = window.location.pathname.includes("genshindle-helper")
    ? "/genshindle-helper/"
    : "./";
  const csvPath = basePath + "List Genshin Impact Characters.csv";
  ...
  fetch(csvPath)
    ...
    .catch((error) => {
      ...
      document.getElementById("loadStatus").innerText =
        "⚠️ Could not auto-load CSV. Upload manually or check console.";
    });
});
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (
      cols.length <=
      Math.max(charIdx, qualIdx, elemIdx, weapIdx, regIdx, verIdx)
    )
      continue;
  ...
  document.getElementById("loadStatus").innerText =
    `🌸 Success: Loaded ${dataset.length} characters dynamically!`;
  buildFiltersUI();
...
function buildFiltersUI() {
  ...
  container.appendChild(
    createFilterGroup("Quality", uniqueQualities, "quality-cb"),
  );
  container.appendChild(
    createFilterGroup("Element", uniqueElements, "element-cb"),
  );
  container.appendChild(
    createFilterGroup("Weapon", uniqueWeapons, "weapon-cb"),
  );
  container.appendChild(
    createFilterGroup("Region", uniqueRegions, "region-cb"),
  );
...
function processLogic() {
  ...
  const userMaxVer =
    parseFloat(document.getElementById("maxVersion").value) || 999;

  let pool = dataset.filter((target) => {
    const qMatch = activeQualities.includes(
      target.Quality.toString().toLowerCase(),
    );
...
function calculateBestNextGuess(pool) {
  ...
  if (pool.length === 0) {
    recArea.innerHTML =
      "<p style='color:#d32f2f; font-weight:bold;'>No characters match the current filters. Please re-check your options.</p>";
    return;
  }
  if (pool.length === 1) {
  ...
  for (let candidate of dataset) {
    ...
    for (let target of pool) {
      ...
      const e =
        target.Element.toLowerCase() === candidate.Element.toLowerCase();
      ...
      else if (parseFloat(target.Version) < parseFloat(candidate.Version))
        v = "down";
